...
$form = mysqli_fetch_assoc($userResult);
    ...
    if (empty($errors)) {
        if ($newPassword !== '') {
            ...
            $stmt = mysqli_prepare(
                $conn,
                "UPDATE users 
                 SET role_id = ?, full_name = ?, username = ?, email = ?, password = ?, phone = ?, status = ?
                 WHERE id = ?"
            );
            ...
            $stmt = mysqli_prepare(
                $conn,
                "UPDATE users 
                 SET role_id = ?, full_name = ?, username = ?, email = ?, phone = ?, status = ?
                 WHERE id = ?"
            );
            mysqli_stmt_bind_param(
                $stmt,
        ...
        $errors[] = "Không thể cập nhật người dùng. Username hoặc email có thể đã tồn tại.";
    }
}
?>
<!doctype html>
...
<head>
    ...
    <title>Sửa người dùng - Hương Việt</title>
